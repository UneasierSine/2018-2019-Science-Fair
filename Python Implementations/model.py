...
from keras.layers import Dense, Conv2D, Flatten, Concatenate, Conv3D
...
train_dir = "C:/Users/panka/Desktop/Aditya/image data 2018-19/Training"
#get labels pickle and convert to dataframe then sort by the filename to go along with the images
train_labels_file = "C:/Users/panka/Desktop/Aditya/image data 2018-19/Training_Input.pkl"
...
test_generator = train_datagen.flow_from_dataframe(dataframe=test_labels,directory=test_dir,target_size=(72,128),x_col='Filename',y_col=['Right Ankle x','Right Knee x','Right Hip x','Left Hip x','Left Knee x','Left Ankle x','Pelvis x','Thorax x','Upper Neck x','Head Top x','Right Wrist x','Right Elbow x','Right Shoulder x','Left Shoulder x','Left Elbow x','Left Wrist x','Right Ankle y','Right Knee y','Right Hip y','Left Hip y','Left Knee y','Left Ankle y','Pelvis y','Thorax y','Upper Neck y','Head Top y','Right Wrist y','Right Elbow y','Right Shoulder y','Left Shoulder y','Left Elbow y','Left Wrist y'],class_mode='other',batch_size=32)

#create model
model = Sequential()

#add model layers
model.add(Conv2D(64, data_format="channels_last", kernel_size=3, input_shape=(72,128,3), activation='relu'))
...
#train the model
model.fit_generator(train_generator,epochs=50,validation_data=test_generator, steps_per_epoch=1000, validation_steps=1000)
